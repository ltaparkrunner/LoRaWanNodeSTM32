...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  /* USER CODE BEGIN 6 */
	...
	//bufPoint = Buf + *Len; /* Eiiao aooa?a ia -1 */
	RingBufferEnd += *Len;
	SumRecordLength += *Len;
	if (RingBufferEnd >= BufferLast256) /* aooa? aeecie e ia?aiieiaie? */
	{
			if(RingBufferBegin > (RingBufferEnd - BufferLast256))	/* e anou naiaiaiia ianoi a ia?aea*/
			{
			...
			else return USBD_FAIL;	/* aooa? ia?aiieiai e ianoa a ia?aea iao */
	}
	//////////////
	//RingBufferEnd = APP_RX_DATA_SIZE -15;
	////////////
	if (RingBufferEnd > RingBufferBegin) USBD_CDC_SetRxBuffer(&hUsbDeviceFS, &UserRxBufferFS[RingBufferEnd]); // iiaia niiauaiea ieoai a eiiao aooa?a
				...
				else return USBD_FAIL;	/* aooa? ia?aiieiai */
...
uint8_t CheckTransmit()
{
	...
	uint8_t result = USBD_OK;
	/* i?iaa?ea, ?oi ia caiyoi e io?aaioea ioeaie  USBD_BUSY*/
